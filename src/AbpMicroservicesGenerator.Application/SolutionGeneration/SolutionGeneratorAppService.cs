    ...
    public class SolutionGeneratorAppService : AbpMicroservicesGeneratorAppService, ISolutionGeneratorAppService
    {
        ...
        private async Task CreateProjectStructureAsync(string solutionPath, SolutionGenerationRequest request)
        {
            ...
            foreach (var service in request.Microservices)
            {
                ...
                var srcFolderPath = Path.Combine(servicePath, "src");


                await GenerateMicroserviceProjectsAsync(srcFolderPath, request, service, "src", solutionPath);
        ...
        {
            if (subFolderType != "src")
                return; // فقط ننشئ الـ Microservice باستخدام abp new مرة واحدة من داخل src

            Logger.LogInformation($"Generating microservice '{microservice.Name}' using ABP CLI");

            var serviceName = $"{request.CompanyName}.{microservice.Name}"; // مثل EMPS.CustomerService
            var outputPath = Path.Combine(mainSolutionDirectory, "services", microservice.Name);

            var psi = new ProcessStartInfo
            {
                FileName = "abp",
                Arguments = $"new {serviceName} -t module --no-ui -o \"{outputPath}\"",
            ...
            {
                Logger.LogError($"❌ Failed to create microservice using ABP CLI: {error}");
                return;
            }

            Logger.LogInformation($"✅ Successfully created microservice using ABP CLI: {output}");

            // 🧹 قبل إضافة المشاريع، نظف الزوائد من المجلد
            await CleanUpUnwantedProjectsAsync(outputPath, $"{request.CompanyName}.{microservice.Name}");

            // أضف المشاريع الناتجة إلى الحل الرئيسي
            var mainSolutionPath = Path.Combine(mainSolutionDirectory, $"{request.SolutionName}.sln");

            // ابحث عن كل csproj داخل هذا المسار
            var csprojFiles = Directory.GetFiles(outputPath, "*.csproj", SearchOption.AllDirectories);
            foreach (var csproj in csprojFiles)
            {
                // نحاول استخراج src أو host أو test من مسار المشروع
                var projectDirectory = Path.GetDirectoryName(csproj);
                var folderType = "src"; // الافتراضي

                if (projectDirectory != null)
                {
                    if (projectDirectory.Contains($"{Path.DirectorySeparatorChar}host{Path.DirectorySeparatorChar}"))
                        folderType = "host";
                    else if (projectDirectory.Contains($"{Path.DirectorySeparatorChar}test{Path.DirectorySeparatorChar}"))
                        folderType = "test";
                }

                var solutionFolder = $"services\\{microservice.Name}\\{folderType}";
                await AddProjectToSolutionAsync(mainSolutionPath, solutionFolder, csproj);
            }
        }
        ...
        private async Task GenerateSolutionFileAsync(string servicePath, SolutionGenerationRequest request, MicroserviceRequest microservice)
        {
            ...
            foreach (var subFolder in subFolders)
            {
                ...
                var csprojFiles = Directory.GetFiles(subFolderPath, "*.csproj", SearchOption.AllDirectories);
                foreach (var csprojFile in csprojFiles)
                {
                    ...
                    var relativeProjectPath = Path.GetRelativePath(servicePath, csprojFile).Replace('/', '\\');
        ...
        private async Task CleanUpUnwantedProjectsAsync(string servicePath, string solutionFileName)
        {
            var slnPath = Path.Combine(servicePath, $"{solutionFileName}.sln");
            if (!File.Exists(slnPath))
            {
                Logger.LogWarning($"❌ Solution file not found: {slnPath}");
                return;
            }

            var folderPatterns = new[]
            {
        "*Installer",
        "*Host.Shared",
        "*MongoDB",
        "*MongoDB.Tests",
        "*Service.AuthServer",
        "*Service.Blazor.WebAssembly.Bundling"
    };

            foreach (var pattern in folderPatterns)
            {
                var folders = Directory.GetDirectories(servicePath, pattern, SearchOption.AllDirectories);
                foreach (var folder in folders)
                {
                    try
                    {
                        // ابحث عن جميع ملفات .csproj في هذا المجلد
                        ...
                        foreach (var csprojFile in csprojFiles)
                        {
                            ...
                            {
                                FileName = "dotnet",
                                Arguments = $"sln \"{slnPath}\" remove \"{csprojFile}\"",
                                RedirectStandardOutput = true,
                                RedirectStandardError = true,
                                UseShellExecute = false,
                                CreateNoWindow = true
                            ...
                            if (process.ExitCode == 0)
                                Logger.LogInformation($"✅ Removed project from solution: {csprojFile}");
                            else
                                Logger.LogWarning($"⚠️ Failed to remove project: {csprojFile} | Error: {error}");
                        }

                        // حذف المجلد بعد إزالة المشاريع من .sln
                        Directory.Delete(folder, recursive: true);
                        Logger.LogInformation($"🗑️ Deleted folder: {folder}");
                    }
                    catch (Exception ex)
                    {
                        Logger.LogWarning($"⚠️ Failed to delete folder: {folder} | {ex.Message}");
                    }
                }
            }
        }
